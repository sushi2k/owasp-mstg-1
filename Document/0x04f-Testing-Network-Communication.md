...
- If an app does not implement pinning, this shouldn't be reported as a vulnerability. However, if the app must verify against [MAS-L2](https://mas.owasp.org/Profiles/MAS-L2/) it must be implemented.
...
Pinning is a recommended practice, especially for [MAS-L2](https://mas.owasp.org/Profiles/MAS-L2/) apps. However, developers must implement it exclusively for the endpoints under their control and be sure to include backup keys (aka. backup pins) and have a proper app update strategy.
